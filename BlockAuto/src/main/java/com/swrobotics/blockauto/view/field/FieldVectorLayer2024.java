...
public final class FieldVectorLayer2024 implements FieldLayer {
    // Defined dimensions
    private static final float fieldWidth = (float) FieldView.WIDTH;
    private static final float fieldHeight = (float) FieldView.HEIGHT;
    ...
    private void drawFieldHalf(PGraphics g, float strokeMul, boolean isRedAlliance) {
        if (isRedAlliance) {
            g.pushMatrix();
    ...
    @Override
    public void draw(PGraphics g, float strokeMul) {
        if (!show.get())
            return;

        drawFieldHalf(g, strokeMul, false);
        drawFieldHalf(g, strokeMul, true);
